...
def contacts_parser():
    ...
    file_route='./facebook-'+user+'/html/'+info_type["contacs"]

    #Data parser
    #Extracting the file
    soup=BeautifulSoup(open(file_route),'lxml')

    contacts_block=soup.find_all('tr') #It's only takes real comments, not share or likes. To take this raw info, use the dates_text instead
    #dates_text=soup.find_all('div',class_="meta") #Used for taking addional info like likes, shared links or others. No formated info

    for contact in contacts_block:
        if(contact.td):
            print(contact.td.get_text())

            if(contact.td.next_sibling):
                contacts_number=(contact.td.next_sibling.span.find_all('li'))
                for contact_n in contacts_number:
                    #print(contact_n.get_text())
                    cadena=""
                    if ("Contact" in contact_n.get_text()):
                        cadena=contact_n.get_text()
                        cadena=cadena.replace("Contact","")
                        cadena=cadena.strip()#Deletes first and last spaces
                    print(cadena)
        print("\n\n")
